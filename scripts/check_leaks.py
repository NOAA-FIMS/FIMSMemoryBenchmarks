...
def run(ref, output):
    ...
    if os.getenv('LEAK_CHECK', '1') != '1':
        ...
        try:
            ...
            command = [str(Path(r_home) / 'bin/exec/R'), '--vanilla', '--slave', '-e',
                       "source('R/setup_FIMS.R'); invisible(setup_fims_model(mode = 'validation')); gc(); writeLines('completed', Sys.getenv('FIMS_LEAK_COMPLETED')); quit(save='no', status=0)"]
            if host == 'Darwin':
