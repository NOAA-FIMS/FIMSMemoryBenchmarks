...
def main() -> int:
    ...
    for run in runs:
        ...
        for metric, unit, value in (
            ("maximum_rss", "bytes", profile.maximum_rss),
            ("peak_footprint", "bytes", profile.peak_footprint),
            ("elapsed_seconds", "seconds", profile.elapsed_seconds),
            ("user_seconds", "seconds", profile.user_seconds),
            ("system_seconds", "seconds", profile.system_seconds),
            ("page_reclaims", "count", profile.page_reclaims),
            ("page_faults", "count", profile.page_faults),
            ("swaps", "count", profile.swaps),
        ):
            if value is not None:
                rows.append({"ref": run.ref, "fims_version": run.version, "source": "time",
                             "metric": metric, "unit": unit, "value": value,
                             "path": profile.path.name})
        rows.append({"ref": run.ref, "fims_version": run.version, "source": "instruments",
                     "metric": "trace_status", "unit": "text", "value": run.trace_status,
                     "path": run.trace_path.name})
        totals = {}
        for values in run.allocations.values():
        ...
        for key, value in totals.items():
            rows.append({"ref": run.ref, "fims_version": run.version, "source": "instruments",
                         ...
                         "value": value, "path": run.trace_path.name})
        for origin, values in (run.allocation_origins or {}).items():
            rows.append({"ref": run.ref, "fims_version": run.version, "source": "instruments",
                         "metric": f"origin_bytes:{origin}", "unit": "bytes",
