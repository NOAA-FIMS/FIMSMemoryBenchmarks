...
# Optional "--tidy-out PATH": the same parse that renders the report, as rows,
# for results.tsv. Removed from args so the positional contract is unchanged.
tidy_out <- ""
flag <- match("--tidy-out", args)
if (!is.na(flag) && length(args) > flag) {
  tidy_out <- args[[flag + 1L]]
  args <- args[-c(flag, flag + 1L)]
}
...
if (nzchar(tidy_out)) {
  rows <- do.call(rbind, lapply(results, function(result) {
    values <- list(
      ...
      final_gradient_max = if (is.null(result$final_gradient)) NA_real_ else max(abs(result$final_gradient)),
    ...
    data.frame(
      ref = result$ref, fims_version = "", stage = "validation", teardown = "",
      size = if (is.null(result$model_size)) "" else result$model_size,
      round = "", iteration = "", source = "validation",
      metric = names(values),
      unit = ifelse(grepl("seconds$", names(values)), "seconds",
             ifelse(grepl("objective|gradient", names(values)), "value", "count")),
      value = unlist(lapply(values, function(v) as.character(v[[1]]))),
      path = "", stringsAsFactors = FALSE
    )
  }))
  utils::write.table(rows, tidy_out, sep = "\t", row.names = FALSE, quote = FALSE)
  message("Validation rows written to ", tidy_out)
}
